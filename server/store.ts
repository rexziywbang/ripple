...
export function createStore(path:string) {
  ...
  db.exec(`PRAGMA journal_mode=WAL; PRAGMA foreign_keys=ON;
    CREATE TABLE IF NOT EXISTS projects (id TEXT PRIMARY KEY, name TEXT NOT NULL, state TEXT NOT NULL);
  ...
  return {
    ...
    list(){return db.prepare('SELECT id,name FROM projects ORDER BY rowid').all() as Array<{id:string;name:string}>;},
    ...
    next():Job|undefined {const row=db.prepare("SELECT id,project_id AS projectId,revision,kind,payload,status FROM jobs WHERE status='pending' ORDER BY created_at,rowid LIMIT 1").get() as Job|undefined;if(row)db.prepare("UPDATE jobs SET status='running' WHERE id=?").run(row.id);return row;},
